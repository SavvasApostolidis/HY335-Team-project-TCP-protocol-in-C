...
void prepare_send_header(microtcp_header_t *header_snd) {
  ...
  header_snd->data_len = htons(header_snd->data_len);
...
int microtcp_connect(microtcp_sock_t *socket, const struct sockaddr *address,
  ...
  tmp_checksum = ntohl(packet_read.checksum);
  ...
  packet_read.checksum = (uint32_t)crc32(checksum_buf, sizeof(checksum_buf));
  ...
  header_snd.checksum =
      (uint32_t)htonl(crc32(checksum_buf, sizeof(checksum_buf)));
  ...
  if (sendto(socket->sd, (void *)&header_snd, sizeof(microtcp_header_t), 0,
    ...
    socket->ack_number =
        packet_read.seq_number +
        s;  // den lambanoume genika acks opote doesnt matter for now
    socket->init_win_size = MICROTCP_WIN_SIZE;
    // socket->sin = (struct sockaddr_in *)address;
    // printf("socket sin_family %u\n", socket->sin->sin_family);
...
int microtcp_accept(microtcp_sock_t *socket, struct sockaddr *address,
  ...
  printf("%zu to check sum pou ebgala me bash to indianes mou \n",
  ...
  // socket->sin->sin_port = ntohs(sin->sin_port);
  // socket->sin->sin_addr.s_addr = ntohs(sin->sin_addr.s_addr);
  // socket->address_len = address_len;
  // printf("sin_family %u\n", sin->sin_family);
  // printf("sin_port %u\n", sin->sin_port);
  // printf("sin_addr %zu\n", sin->sin_addr.s_addr);
...
ssize_t microtcp_send(microtcp_sock_t *socket, const void *buffer,
  ...
  printf("Going to send to %zu ip %u port\n", socket->sin->sin_addr.s_addr,
         htons(socket->sin->sin_port));
  for (i = 0; i < packets_num; i++) {
    ...
    /*Checksums*/
    tmpChecksum = (uint32_t)crc32(send_buffer, sizeof(send_buffer));
    ...
    if (tmp_bytes =
            sendto(socket->sd, (void *)&send_buffer, MICROTCP_MSS, 0,
                   (struct sockaddr *)socket->sin, socket->address_len) == -1) {
...
ssize_t microtcp_recv(microtcp_sock_t *socket, void *buffer, size_t length,
  ...
  /*Allocate memory to read the packet*/
  header_rcv = (microtcp_header_t *)malloc(sizeof(microtcp_header_t *));
  printf("Receiving from %u port by socket\n", htons(socket->sin->sin_port));
  printf("Receiving from %zu ip by socket\n", socket->sin->sin_addr.s_addr);
  while (running) {
    /*Waiting fot FIN from D*/
    if (bytes_transfered = recvfrom(socket->sd, rcvbuf, MICROTCP_MSS, 0,
    ...
    tmp_calc_checksum = (uint32_t)crc32(checksum_buf, sizeof(checksum_buf));
    ...
    if (packet_read.data_len > bytes_transfered - sizeof(microtcp_header_t)) {
      ...
      total_bytes_lost +=
          packet_read.data_len - bytes_transfered - sizeof(microtcp_header_t);
      continue; /*tmp*/
    }
    if (socket->ack_number < packet_read.seq_number) { /*xa8hke paketo*/
      socket->packets_lost = packet_read.seq_number - socket->ack_number;
      // total_bytes_lost += packet_read.data_len*(packet_read.seq_number -
      // socket->ack_number);
    }
    ...
    if (packet_read.control == ACK) {
      ...
      socket->ack_number =
          packet_read.seq_number++; /*proxoraw kata 1 gia na kanw expect swsto
                                       1h fash*/
      ...
      socket->packets_received++;
      socket->ack_number = packet_read.seq_number;
      /*todo isws mas poun oti 8a erxontai data sto shutdown diabaseta ola,isws
       * looparw th rcvfrom h steilw acks*/
      /*todo xrono*/
      break;
    }
